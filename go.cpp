...
{ 
	...
	//	Mat trainingDataMat(train_sample_number, numberofbins, CV_32FC1, trainingData);
		...
		//*********** HERE COMES THE PART WHERE YOU FILL x_space **************//

		prob.l = train_sample_number;
		svm_node** x = Malloc(svm_node*,train_sample_number);
	
		 int r, c;
    for (r=0; r<train_sample_number; r++) //180
    {

		  svm_node* x_space = Malloc(svm_node,numberofbins+1);

        for (c=0; c<numberofbins; c++)//436
        {
			 x_space[c].index = c;
			 x_space[c].value = trainingData[r][c];

		}
	 x_space[numberofbins].index = -1; 
	 x[r] = x_space;
	 ...
	 prob.x = x;
	  ...
	  for(r=0; r<train_sample_number; r++){
		  prob.y[r] = labels_array[r];}

		svm_model *model = svm_train(&prob,&param);

		mysvmstream << svmfile << trainnumber << txttype;
		string mysvmfilename = mysvmstream.str();
		const char *cstrs = mysvmfilename.c_str();
		svm_save_model(cstrs,model);
    ...
    svmVec = (struct svm_node *)malloc((numberofbins+1)*sizeof(struct svm_node));
    double *predictions = new double[test_sample_number];
//    float *dataPtr = testing_data.ptr<float>(); // Get data from OpenCV Mat
    double prob_est[10];  // Probability estimation
     FILE *foutp = fopen("predictions.txt","w");

    for (r=0; r<test_sample_number; r++)
    {
        for (c=0; c<numberofbins; c++)
        {
            svmVec[c].index = c;  // Index starts from 1; Pre-computed kernel starts from 0
            svmVec[c].value = samples[r][c];
        }
        svmVec[numberofbins].index = -1;   // End of line
       
        if(svm_check_probability_model(model))
        {
            predictions[r] = svm_predict_probability(model, svmVec, prob_est);
            printf("%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\n", predictions[r], prob_est[0], prob_est[1],prob_est[2], prob_est[3], prob_est[4], prob_est[5],prob_est[6], prob_est[7], prob_est[8], prob_est[9]);
        	fprintf(foutp, "%f %f %f %f %f %f %f %f %f %f %f\n", predictions[r], prob_est[0], prob_est[1],prob_est[2], prob_est[3], prob_est[4], prob_est[5],prob_est[6], prob_est[7], prob_est[8], prob_est[9]);

		}
        else
        {
            predictions[r] = svm_predict(model, svmVec);
            printf("%f\n", predictions[r]);
        }
    }
	...
	//------ test without train -------//
		...
		const char *MODEL_FILE = "my_svm_10.txt";
		struct svm_model *SVMModel;
		SVMModel = svm_load_model(MODEL_FILE);
